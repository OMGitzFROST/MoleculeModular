...
public final class HoverAdapter implements com.moleculepowered.api.adapter.HoverAdapter<HoverEvent>
{
    ...
    public @NotNull HoverEvent showText(String value) {
        return new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(value).create());
    }
    ...
    public @NotNull <V> HoverEvent showItem(V item) {
        ...
        if (!(item instanceof org.bukkit.inventory.ItemStack))
            throw new IllegalArgumentException("The object you provided is not an ItemStack");
        ...
        return new HoverEvent(HoverEvent.Action.SHOW_ITEM, new BaseComponent[]{new TextComponent(itemStack.save(tag).toString())});
    }
    ...
    public @NotNull <V> HoverEvent showEntity(V entity) {
        ...
        String entityType   = ((Entity) entity).getType().toString();
        String entityID     = ((Entity) entity).getUniqueId().toString();
        String entityName   = ((Entity) entity).getName();
        String entityFormat = "{id:" + entityID + ",name:" + entityName + ",type:" + entityType + "}";

        return new HoverEvent(HoverEvent.Action.SHOW_ENTITY, new ComponentBuilder(entityFormat).create());
    }
